...
async function searchItunes(query: string): Promise<ItunesTrack[]> {
  try {
    const res = await fetch(
      `/api/music/search?q=${encodeURIComponent(query)}&limit=8`
    );
    if (!res.ok) return [];
...
export default function AddSongPage() {
  ...
  const handleSearch = async () => {
    ...
    setResults(res);
    setSearching(false);
    if (res.length === 0) toast.error("Nie znaleziono piosenek. Spróbuj innego zapytania.");
  };
  ...
  const handleSave = async () => {
    ...
    const slug = `${selected.trackName}-${selected.artistName}`
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/[^a-z0-9]+/g, "-")
